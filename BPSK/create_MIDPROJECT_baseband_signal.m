...
symbolrate = 1e6; % Symbol rate in Hz. Chosen to be a divisor of TXusersamplingrate for ease of implementation. ** Try to increase this substantially for the final project **

Ex = 0.01; % Symbol energy. ** Change this for the final project **

lenmessagebits = 1; % Total number of message bits. ** Need more bits in final project **
lenpilotbits = 500; % Number of pilot bits (includes timing and frame sync bits). ** Change this for the final project **

b = 1; % Since BPSK modulation. ** Must be variable in final project **
numinfobits = 1; % k=1 for repetition code. ** Choose 1 if convolutional code used in the final project **
numcodedbits = 20; % n of repetition code. So code rate is k/n. ** Optionally implement a convolutional code in the final project **
...
constellation = sqrt(Ex)*[-1; 1]; % BPSK constellation symbols. ** Need M-QAM in final project **
r = numinfobits/numcodedbits;
disp(['Using repetition-coded BPSK modulation of code rate r = ' num2str(r) ''])
...
% **********************************************************
% Create packet bits = (pilot bits, message bits)
...
% Create coded bits (repetition code used here). ** Optionally use convolutional code in final project **
codedbits = repmat(messagebits, numcodedbits,1);
...
lenpilotsymbols = lenpilotbits; % Due to BPSK modulation

% BPSK modulation for coded bits. ** Use M-QAM in final project **
xkmessage = constellation(codedbits+1); % BPSK modulation
lencodedsymbols = length(codedbits); % Due to BPSK modulation
