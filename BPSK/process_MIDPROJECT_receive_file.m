...
% c = 1;
% plot(abs(rxsigupsampled))
% plot(abs(pilotsignal))
% plot(abs(CTtau))
% for i = e
%     possync = i;
...
possync = indx-lenpilotsamples; 
% This marks the beginning of the sync pilot in the received signal. 
%** This is intentionally INCORRECT. You must calculate the correct offset for the final project **
tp = abs(corrcoef(rxsigupsampled(possync:possync+lenpilotsamples-1),pilotsignal));
synccorrcoef = tp(2,1);
...
zt = conv(flipud(pt),yt) * (1/Tsamples); 
% 1/Tsamples is the dt in the integration (since the integral has been replaced by a sum here)
...
filtertimeoffset = lenptsamples; %    ceil(lenptsamples*1.2)+161; 
% ** This is intentionally INCORRECT. 
%You must calculate the correct offset for the final project **
...
stem(sign(real(zk(1:30))),'b');
hold on;
stem(.5*sign(real(bits(1:30)-.5)), 'r');
...
%** This is intentionally INCORRECT. You must calculate the correct estimate for the final project **
...
vk = (1/h0)*zk; % Equalized samples. 
%** This is intentionally INCORRECT. You must calculate the correct equalization for the final project **
...
xkpilothat = sqrt(Ex)*sign(real(vkpilot)); % Minimum distance detector for BPSK
...
messagebitshat = (vkcodedmatched>0); % BPSK decoding
...
BER = mean(messagebitshat ~= messagebits);
